    ...
    <div className="border border-[var(--border)] p-6 rounded-2xl shadow-md hover:shadow-lg hover:border-current/30 hover:-translate-y-0.5 transition-all duration-300 bg-[var(--background)] flex flex-col">
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {f.domainLabels.map(d => (
          <span key={d} className="text-xs px-2 py-0.5 border border-[var(--border)] rounded-md opacity-60 font-medium">{d}</span>
        ))}
      </div>
      {!f.bio ? (
        <p className="text-xs opacity-30 italic mb-4 leading-relaxed">Bio en cours de rédaction — expert terrain reconnu.</p>
      ) : (
        <p className="text-base opacity-60 mb-4 leading-relaxed">{f.bio}</p>
      )}
